...
export async function loadPage() {
  ...
  document.getElementById("addQuestBtn").addEventListener("click", async (e) => {
    await createNewQuest(e, questForm, questModal)
  });
}

export async function createNewQuest(e, questForm, questModal) {
...
export function addQuestCard( questId, title, category, description, points=3, completed=false) {
  ...
  toggle.addEventListener('change', async () => {
    await completedQuest(questId, toggle);
  })

  // Add edit functionality
  const editBtn = card.querySelector('.edit-btn');
  editBtn.addEventListener('click', async () => {
    await editQuest(questId, editBtn);
  });
...
export async function completedQuest(questId, toggle) {
...
export async function editQuest(questId, editBtn) {
